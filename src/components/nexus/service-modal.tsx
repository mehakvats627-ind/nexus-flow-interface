...
export function ServiceModal({
  ...
  const running = ["request", "payment-required", "processing", "paid", "delivered"].includes(
    flow.stage,
  );
      ...
      <DialogContent className="glass-panel max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <dl className="grid grid-cols-2 gap-3 rounded-lg border border-border bg-secondary/40 p-4 text-xs">
          <div>
            ...
            <dd className="mt-0.5">{service.provider}</dd>
          </div>
          <div>
            <dt className="text-muted-foreground">Price</dt>
            <dd className="mt-0.5 font-display text-primary">{formatUSD(service.price)} USDC</dd>
          </div>
          <div>
            <dt className="text-muted-foreground">Remaining budget</dt>
            <dd className="mt-0.5">{formatUSD(budget.remaining)}</dd>
          </div>
          <div>
        ...
        {blocked && (
          <div className="relative overflow-hidden rounded-lg border border-destructive/40 bg-destructive/10 p-5 text-center">
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              ...
              Remaining:{" "}
              <strong className="text-foreground">{formatUSD(flow.remainingAtRequest)}</strong>
            </p>
        ...
        {duplicate && (
          <div className="rounded-lg border border-warning/40 bg-warning/10 p-5 text-center">
            ...
            <p className="mt-3 font-display text-lg font-semibold tracking-[0.1em] text-warning">
              DUPLICATE REQUEST
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Already processed. No second charge.
            </p>
          ...
          <div className="rounded-lg border border-success/30 bg-success/8 p-4">
            ...
            <dl className="mt-3 space-y-2 text-xs">
              <div className="flex justify-between gap-3">
                ...
                <dd>{formatUSD(flow.receipt.amount)} USDC</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-muted-foreground">Request ID</dt>
                <dd>
                  <CopyValue value={flow.receipt.requestId} label="Request ID" />
                </dd>
              </div>
              <div className="flex justify-between gap-3">
                ...
                <dd>
                  <CopyValue
                    value={flow.receipt.txHash}
                    display={shortHash(flow.receipt.txHash, 6)}
              ...
              <div className="flex justify-between gap-3">
                ...
                <dd>
                  <CopyValue
                    value={flow.receipt.contentHash}
                    display={shortHash(flow.receipt.contentHash, 6)}
                    label="Content hash"
                  />
                </dd>
              </div>
