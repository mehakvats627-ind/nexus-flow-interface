  ...
  head: () => ({
    meta: [
      { title: "Services — NEXUS" },
      {
        name: "description",
        content:
          "Translation, code generation and image generation the NEXUS agent can buy on demand.",
      },
    ...
    <AppShell breadcrumb="Services">
        ...
        <PanelTitle
          ...
          aside={
            <span className="text-[10px] text-muted-foreground">
              Remaining {formatUSD(budget.remaining)}
            </span>
          }
        />
      ...
      <Panel delay={120}>
        ...
        <div className="grid gap-4 p-5 text-xs text-muted-foreground sm:grid-cols-3">
          <p>
            Every request is priced up front and checked against the remaining budget before any
            payment is settled.
          </p>
          <p>
            Requests above the remaining budget are blocked and recorded — no charge ever reaches
            the provider.
          </p>
          <p>
            Replaying a Request ID that already settled returns the original receipt instead of
            charging twice.
          </p>
        </div>
      </Panel>
