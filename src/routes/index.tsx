...
import {
  ArrowRight, Activity as ActivityIcon, Bot, Code2, FileText, History, Image as ImageIcon,
  Languages, Lock, Receipt as ReceiptIcon, ShieldAlert, ShieldCheck, WalletCards, Zap,
} from "lucide-react";
...
import {
  AGENT_ADDRESS, formatUSD, getActivities, getBudget, getFlow, getLatestReceipt,
  getServices, getStats, getTransactions, resetDemo, runPurchase, shortHash, timeAgo,
  type ServiceId, type Transaction,
        ...
        <div className="pointer-events-none absolute left-[52%] top-8 z-10 hidden flex-col gap-3 lg:flex">
          {services.map((s, i) => {
            ...
            return (
              ...
              >
                <Icon className="size-3.5 text-primary" />{s.name}
              </span>
            );
          })}
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_1fr_1fr]">
        <Panel>
          <PanelTitle
            ...
            aside={<span className="rounded-full border border-border bg-secondary/60 px-2.5 py-1 text-[10px] text-muted-foreground">USDC</span>}
          />
          <div className="p-5">
        ...
        <Panel delay={80}>
          ...
          <div className="space-y-3 p-5 text-xs">
            {[
              ...
              <div key={k} className="flex items-center justify-between rounded-lg border border-border bg-secondary/40 px-3 py-2.5">
                <span className="text-muted-foreground">{k}</span>
                <span className={`flex items-center gap-1.5 font-medium ${c}`}>
                  <span className="size-1.5 rounded-full bg-current shadow-[0_0_10px_currentColor]" />{v}
                </span>
              </div>
            ))}
            <div className="flex items-center justify-between rounded-lg border border-border bg-secondary/40 px-3 py-2.5">
              <span className="text-muted-foreground">Agent</span>
              <CopyValue value={AGENT_ADDRESS} display={shortHash(AGENT_ADDRESS)} label="Agent address" />
            </div>
          </div>
        </Panel>
          ...
          <div className="space-y-2.5 p-5">
            {services.map((s) => {
              ...
              return (
                ...
                >
                  <span className="grid size-8 place-items-center rounded-md bg-primary/10 text-primary"><Icon className="size-4" /></span>
                  <span className="flex-1">{s.name}</span>
                  <span className={`font-display text-xs ${affordable ? "text-primary" : "text-destructive"}`}>{formatUSD(s.price)}</span>
                </button>
              );
            })}
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <Panel>
          <PanelTitle
            ...
            aside={<span className="text-[10px] text-muted-foreground">{FLOW_LABELS[flow.stage]}</span>}
          />
          <div className="space-y-3 p-5">
            {FLOW_ORDER.map((stage, i) => {
              const state = flowIdx < 0 ? "todo" : i < flowIdx ? "done" : i === flowIdx ? "current" : "todo";
              return (
                <div key={stage} className="flex items-center gap-3 text-xs">
                  <span
                    className={`size-2.5 rounded-full ${
                      state === "done" ? "bg-primary" : state === "current" ? "bg-primary shadow-glow animate-pulse" : "bg-secondary"
                    }`}
                  />
                  <span className={state === "todo" ? "text-muted-foreground" : ""}>{FLOW_LABELS[stage]}</span>
                </div>
              );
            })}
            {flow.stage === "idle" && (
              <p className="pt-1 text-[11px] text-muted-foreground">Start a service request to watch the payment flow live.</p>
            )}
          </div>
        </Panel>
          ...
          <div className="p-5 text-center">
            {lastBlocked ? (
              <div className="relative overflow-hidden rounded-lg border border-destructive/40 bg-destructive/10 p-5">
                ...
                <p className="text-xs uppercase tracking-[0.2em] text-destructive/80">Budget Exceeded</p>
                <p className="mt-3 text-xs text-muted-foreground">
                  Requested: <strong className="text-foreground">{formatUSD(lastBlocked.amount)}</strong>
                  <span className="mx-2 text-border">|</span>
                  Remaining: <strong className="text-foreground">{formatUSD(budget.remaining)}</strong>
                </p>
              ...
              <div className="rounded-lg border border-success/30 bg-success/8 p-6">
                ...
                <p className="mt-3 font-display text-sm font-semibold text-success">All payments within budget</p>
          ...
          <div className="p-5 text-xs">
            {receipt ? (
              <dl className="space-y-2.5">
                <Row k="Request ID" v={<CopyValue value={receipt.requestId} label="Request ID" />} />
                <Row k="Service" v={receipt.service} />
                <Row k="Payment" v={`${formatUSD(receipt.amount)} USDC`} />
                <Row k="Transaction Hash" v={<CopyValue value={receipt.txHash} display={shortHash(receipt.txHash, 6)} label="Transaction hash" />} />
                <Row k="Content Hash" v={<CopyValue value={receipt.contentHash} display={shortHash(receipt.contentHash, 6)} label="Content hash" />} />
            ...
            {transactions.map((tx) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block font-mono text-[10px] text-muted-foreground">{tx.requestId} · {timeAgo(tx.createdAt)}</span>
                </span>
          ...
          <ol className="space-y-4 p-5">
            {activities.map((a) => (
              <li key={a.id} className="relative pl-5 text-xs">
                ...
                <p className="mt-0.5 text-[10px] text-muted-foreground/70">{timeAgo(a.createdAt)}</p>
              </li>
            ))}
          </ol>
        </Panel>
