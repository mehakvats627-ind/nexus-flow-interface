...
function safeChecksum(addr: string, fallback: string): string {
  try { return ethers.getAddress(addr.toLowerCase()); } catch { return fallback; }
}
...
export interface SepoliaNetworkStatus {
  connected: boolean; chainId: number; networkName: string; rpcUrl: string; latestBlock: number;
  agentAddress: string; agentEthBalance: string; agentUsdcBalance: string; ownerAddress: string;
  contractAddress: string; contractBudget: number; contractSpent: number; contractRemaining: number;
  isAgentAuthorized: boolean; usdcAddress: string; hasPrivateKey: boolean; error?: string;
}

export interface OnChainSettlementResponse {
  success: boolean; status: "Success" | "Blocked" | "Duplicate" | "Failed";
  txHash: string | null; contentHash: string | null; blockNumber?: number; gasUsed?: string; error?: string;
  auditRecord?: { requestId: string; agent: string; amount: number; serviceId: string; provider: string; contentHash: string; timestamp: number; txHash: string; blockNumber?: number };
}
...
export class ServerBlockchainService {
  ...
  constructor() {
    ...
    if (AGENT_PRIVATE_KEY && AGENT_PRIVATE_KEY.length >= 64) {
      try {
      ...
      } catch (error) { console.warn("[Server Blockchain] Invalid AGENT_PRIVATE_KEY format:", error); }
    }
  ...
  public async getStatus(): Promise<SepoliaNetworkStatus> {
    try {
      ...
      let totalBudget = 10, totalSpent = 0, isAgentAuthorized = false, onChainOwner = OWNER_ADDRESS;
      try {
        const [rawBudget, rawSpent, authAgent, owner] = await Promise.all([
          this.contract.getBudgetSummary(), this.contract.getBudgetSummary().then((v: [bigint, bigint]) => v[1]),
          this.contract.agent(), this.contract.owner(),
        ]);
        totalBudget = Number(ethers.formatUnits(rawBudget[0] ?? rawBudget, 6));
        totalSpent = Number(ethers.formatUnits(rawSpent, 6));
        isAgentAuthorized = authAgent.toLowerCase() === activeAgent.toLowerCase();
        onChainOwner = owner;
      } catch {
        // A reachable RPC does not imply a valid contract. Keep authorization false.
      }

      return { connected: true, chainId: Number(network.chainId) || CHAIN_ID, networkName: "Sepolia Testnet", rpcUrl: SEPOLIA_RPC_URL,
        latestBlock: blockNumber, agentAddress: activeAgent, agentEthBalance: ethBal, agentUsdcBalance: usdcBal,
        ownerAddress: onChainOwner, contractAddress: CONTRACT_ADDRESS, contractBudget: totalBudget, contractSpent: totalSpent,
        contractRemaining: Math.max(0, totalBudget - totalSpent), isAgentAuthorized, usdcAddress: USDC_ADDRESS, hasPrivateKey: !!this.signer };
    } catch (err) {
      return { connected: false, chainId: CHAIN_ID, networkName: "Sepolia Testnet", rpcUrl: SEPOLIA_RPC_URL, latestBlock: 0,
        agentAddress: AGENT_ADDRESS, agentEthBalance: "0.0", agentUsdcBalance: "0.0", ownerAddress: OWNER_ADDRESS,
        contractAddress: CONTRACT_ADDRESS, contractBudget: 0, contractSpent: 0, contractRemaining: 0,
        isAgentAuthorized: false, usdcAddress: USDC_ADDRESS, hasPrivateKey: !!this.signer,
  ...
  public async settlePaymentOnChain(requestId: string, serviceId: string, provider: string, amount: number, deliveredContent: string): Promise<OnChainSettlementResponse> {
    ...
    try {
      if (await this.contract.isProcessed(requestId)) return { success: false, status: "Duplicate", txHash: null, contentHash: null, error: `DuplicateRequest("${requestId}"): Request was already processed on-chain.` };
    } catch {}
    try {
      const remaining = Number(ethers.formatUnits(await this.contract.getRemainingBudget(), 6));
      if (amount > remaining) return { success: false, status: "Blocked", txHash: null, contentHash: null, error: `BudgetExceeded: Requested amount ${amount} USDC exceeds smart contract remaining budget of ${remaining} USDC.` };
    } catch {}
    ...
    try {
      ...
      const allowance: bigint = await this.usdcContract.allowance(agentAddress, CONTRACT_ADDRESS);
      ...
      const tx = await contractWithSigner.settlePayment(requestId, serviceId, provider, amountInUnits, contentHash);
      ...
      const message = String((err as { reason?: string; shortMessage?: string; message?: string })?.reason || (err as { shortMessage?: string })?.shortMessage || (err as { message?: string })?.message || err);
  ...
  public async getPaymentDetails(requestId: string) {
    try { const record = await this.contract.getPayment(requestId); return { requestId: record._requestId, agent: record._agent, amount: Number(ethers.formatUnits(record._amount, 6)), serviceId: record._serviceId, provider: record._provider, contentHash: record._contentHash, timestamp: Number(record._timestamp) * 1000 }; }
    catch { return null; }
  }
}
